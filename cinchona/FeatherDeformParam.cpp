...
FeatherDeformParam::FeatherDeformParam()
{
	...
	m_sideInterp->setFilterLength(.33f);
	m_upInterp = new gpr::GPInterpolate<float>();
	m_upInterp->create(4,1,3);
	m_upInterp->setFilterLength(.33f);
}
...
void FeatherDeformParam::set(const Matrix33F * mats)
{
	m_changed = false;
...
bool FeatherDeformParam::isChanged() const
{ return m_changed; }

Vector3F * FeatherDeformParam::rotationSideR(int i)
{ return &m_vecs[i<<1]; }

Vector3F * FeatherDeformParam::rotationUpR(int i)
{ return &m_vecs[(i<<1)+1]; }

const Matrix33F & FeatherDeformParam::rotation(int i) const
{ return m_rots[i]; }

void FeatherDeformParam::learnOrientation()
{
	...
	if(!m_sideInterp->learn() ) {
		std::cout<<"FeatherDeformParam learnOrientation side interpolate failed to learn";
	}
	
	if(!m_upInterp->learn() ) {
		std::cout<<"FeatherDeformParam learnOrientation up interpolate failed to learn";
	}
}
