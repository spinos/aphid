...
{
	feather()->pitchAndCurl(pitchAngle(), curlAngle());
}
...
void MlCalamus::setCurlAngle(const float & x)
{ 
	if(x < 0.f) m_curlAngle = 0.f;
	else m_curlAngle = x;
}

float MlCalamus::pitchAngle() const { return m_rotY; }
void MlCalamus::setPitchAngle(const float & x) 
{ 
	if(x < 0.f) m_rotY = 0.f;
	else m_rotY = x;
}
