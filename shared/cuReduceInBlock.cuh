...
{
    if(NumThreads > 128) {
        if(tid < 128) m[tid] = max(m[tid], m[tid + 128]);
        __syncthreads();
    }
    
    if(NumThreads > 64) {
        if(tid < 64) m[tid] = max(m[tid], m[tid + 64]);
        __syncthreads();
    }
    ...
    if(tid < 8) m[tid] = max(m[tid], m[tid + 8]);

    if(tid < 4) m[tid] = max(m[tid], m[tid + 4]);
    
    if(tid < 2) m[tid] = max(m[tid], m[tid + 2]);

    if(tid < 1) m[tid] = max(m[tid], m[tid + 1]);
}
